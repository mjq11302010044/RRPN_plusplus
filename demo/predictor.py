...
from maskrcnn_benchmark.modeling.roi_heads.rmask_head.inference import Masker as RMasker
...
class ICDARDemo(object):
    ...
    def overlay_class_names(self, image, predictions):
        ...
        scores = predictions.get_field("scores").tolist()
        labels = predictions.get_field("labels").tolist()
        # mask_scores = predictions.get_field("mask_score").tolist()
        labels = [self.CATEGORIES[i] for i in labels]
        boxes = predictions.bbox

        template = "{}: {:.2f} / {:.2f}"
        for box, score, label, m_score in zip(boxes, scores, labels, mask_scores):
            x, y = box[:2]
            s = template.format(label, score, m_score)
            # print("m_score:", score, m_score)
            cv2.putText(
                image, s, (x, y), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255), 1
            )
...
class RRPNDemo(object):
    ...
    def __init__(
        ...
        checkpointer = DetectronCheckpointer(cfg, self.model, save_dir=save_dir)
        _ = checkpointer.load(False, cfg.MODEL.WEIGHT)

        self.transforms = self.build_transform()

        mask_threshold = -1 if show_mask_heatmaps else 0.5
        self.masker = RMasker(threshold=mask_threshold, padding=1)
    ...
    def overlay_class_names(self, image, predictions):
        ...
        if self.cfg.MODEL.MASK_ON:
            ...
            for box, score, label, m_score in zip(boxes, scores, labels, mask_scores):
                ...
                # print("m_score:", score, m_score)
                cv2.putText(
                    image, s, (x, y), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 255, 255), 1
                )
        else:
            template = "{}: {:.2f}"
            for box, score, label in zip(boxes, scores, labels):
