...
from maskrcnn_benchmark.utils.visualize import vis_image, vis_masks
from maskrcnn_benchmark.structures.segmentation_for_rbox import SegmentationMask
import cv2


def get_IC13_dataset(mode, dataset_dir):
    ...
    if mode == 'train':
        cache_pkl = './data_cache/IC13_mask_training.pkl'
    elif mode == 'test':
        cache_pkl = 'IC13_testing.pkl'
    ...
    for image in gt_file_list:
        ...
        for gt_ele in gt_list:
            ...
            if len(gt) > 1:
                ...
                l = np.min(np.array([gt_ind[0], gt_ind[2], gt_ind[4], gt_ind[6]]))
                r = np.max(np.array([gt_ind[0], gt_ind[2], gt_ind[4], gt_ind[6]]))
                t = np.min(np.array([gt_ind[1], gt_ind[3], gt_ind[5], gt_ind[7]]))
                b = np.max(np.array([gt_ind[1], gt_ind[3], gt_ind[5], gt_ind[7]]))

                # order in clock-wise
                poly = [l, t, r, t, r, b, l, b]
        ...
        poly_boxes = np.array(poly_boxes)
...
def get_ICDAR2015_RRC_PICK_TRAIN(mode, dataset_dir):
    ...
    cache_file = './data_cache/IC15_mask_training.pkl'
    ...
    for image in image_list:
        ...
        for gt_line in gt_split:
            ...
            if len(gt_ind) > 3 and '###' not in gt_ind[8]:
                ...
                # print("min:", np.array([gt_ind[0], gt_ind[2], gt_ind[4], gt_ind[6]]))

                gt_ind_int = np.array(gt_ind[:8]).astype(np.int32)

                l = np.min(np.array([gt_ind_int[0], gt_ind_int[2], gt_ind_int[4], gt_ind_int[6]]))
                r = np.max(np.array([gt_ind_int[0], gt_ind_int[2], gt_ind_int[4], gt_ind_int[6]]))
                t = np.min(np.array([gt_ind_int[1], gt_ind_int[3], gt_ind_int[5], gt_ind_int[7]]))
                b = np.max(np.array([gt_ind_int[1], gt_ind_int[3], gt_ind_int[5], gt_ind_int[7]]))
        ...
        poly_boxes.extend(easy_polys)
        poly_boxes.extend(hard_polys[0: int(len(hard_polys) / 3)])
...
def get_ICDAR2017_mlt(mode, dataset_dir):
    DATASET_DIR = dataset_dir
    task = 'double_class'
    prefetched = True if os.path.isfile('./data_cache/ICDAR2017_training_mask_cache.pkl') else False
    im_infos = []
    ...
    if not prefetched:
        ...
        if mode == "train":
            ...
            f_save_pkl = open('./data_cache/ICDAR2017_training_mask_cache.pkl', 'wb')
            pickle.dump(im_infos, f_save_pkl)
            f_save_pkl.close()
            print ("Save pickle done.")
        ...
        if mode == "train":
            f_pkl = open('./data_cache/ICDAR2017_training_mask_cache.pkl', 'rb')
...
def get_ICDAR_LSVT_full(mode, dataset_dir):
    ...
    cache_file = './data_cache/LSVT_mask_' + mode + '.pkl'
    if os.path.isfile(cache_file):
    ...
    for imnum in im_codes:
        ...
        for i in range(inst_num):
            ...
            if illegibility:
                hard_boxes.append([x_ctr, y_ctr, width, height, angle])
                hard_polys.append(polygon)
            else:
                easy_boxes.append([x_ctr, y_ctr, width, height, angle])
                easy_polys.append(polygon)
        ...
        polygons = []
        polygons.extend(easy_polys)
        polygons.extend(hard_polys[0: int(len(hard_polys) / 5)])
        ...
        # gt class that had the max overlap
        max_classes = overlaps.argmax(axis=1)
        if gt_boxes.shape[0] <= 0 or gt_boxes.shape[0] >= 61:
            continue
        # print('gt_boxes:', gt_boxes)
        im_info = {
            'gt_classes': gt_classes,
...
def get_ICDAR_ArT(mode, dataset_dir):
    ...
    cache_file = './data_cache/ArT_mask_' + mode + '.pkl'
    if os.path.isfile(cache_file):
        print('dataset cache found, loading from it...')
        im_infos = pickle.load(open(cache_file, 'rb'))
        print('load done')
    ...
    for imnum in im_codes:
        ...
        for i in range(inst_num):
            ...
            if height * width * (800 / float(im.shape[0])) < 32 * 32 and mode == "train":
                 continue
        ...
        polys = []
        polys.extend(easy_polys)
        polys.extend(hard_polys[0: int(len(hard_polys) / 5)])
        ...
        # gt class that had the max overlap
        max_classes = overlaps.argmax(axis=1)
        if gt_boxes.shape[0] <= 0 or gt_boxes.shape[0] >= 101:
            continue
        # print('gt_boxes:', gt_boxes)
        im_info = {
            'gt_classes': gt_classes,
...
_DEBUG = False
class RotationMaskDataset(torch.utils.data.Dataset):
    CLASSES = (
        "__background__ ",
        "text"
    )
    ...
    def __init__(self, database, use_difficult=False, transforms=None):
        ...
        cls = RotationMaskDataset.CLASSES
    ...
    def map_class_id_to_class_name(self, class_id):
        return RotationMaskDataset.CLASSES[class_id]
    ...
    # def show_masks(self, img, target):
