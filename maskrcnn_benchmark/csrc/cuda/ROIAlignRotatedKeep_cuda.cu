...
template <typename T>
__global__ void RoIAlignRotatedKeepForward(
    const int nthreads,
    const T* input,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    ///////////////////////////////////////////////////////////////////////
    // Modify the bin_size_w to keep the same ratio as original text roi
    T roi_ratio = static_cast<T>(roi_width) / static_cast<T>(roi_height);

    int real_bin_num_in_width = (int)ceil(static_cast<T>(pooled_height) * roi_ratio) + 1;
    real_bin_num_in_width = min(real_bin_num_in_width, pooled_width);

    // Modify the bin_size
    bin_size_w = static_cast<T>(roi_width) / static_cast<T>(real_bin_num_in_width);
    ...
    // If the offset is larger than the ratio, padding them zeros
    if (pw < real_bin_num_in_width) {
...
template <typename T>
__global__ void RoIAlignRotatedKeepBackwardFeature(
    const int nthreads,
    const T* top_diff,
    const int num_rois,
    const T spatial_scale,
...
at::Tensor ROIAlignRotatedKeep_forward_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES(
      input.type(), "ROIAlignRotatedKeep_forward", [&] {
        RoIAlignRotatedKeepForward<scalar_t><<<grid, block, 0, stream>>>(
            output_size,
...
at::Tensor ROIAlignRotatedKeep_backward_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES(
      grad.type(), "ROIAlignRotatedKeep_backward", [&] {
        RoIAlignRotatedKeepBackwardFeature<scalar_t><<<grid, block, 0, stream>>>(
            grad.numel(),
