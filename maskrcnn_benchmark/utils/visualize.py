...
def vis_image(img,
    ...
    for idx in range(len(boxes)):
        if repre == "rbox":
            ...
            rotated_pts = np.array(boxes[idx]).reshape(4, 2)
        # rotated_pts[rotated_pts <= 0] = 1
        # rotated_pts[rotated_pts > img.shape[1]] = img.shape[1] - 1

        if mode == 1:
            draw.line((int(rotated_pts[0, 0]), int(rotated_pts[0, 1]), int(rotated_pts[1, 0]), int(rotated_pts[1, 1])), fill=(0, 255, 0), width=box_width)
            draw.line((int(rotated_pts[1, 0]), int(rotated_pts[1, 1]), int(rotated_pts[2, 0]), int(rotated_pts[2, 1])),
                      fill=(0, 255, 0), width=box_width)
            ...
            draw.line((int(rotated_pts[0, 0]), int(rotated_pts[0, 1]), int(rotated_pts[1, 0]), int(rotated_pts[1, 1])),
                      fill=(255, 255, 0), width=box_width)
            draw.line((int(rotated_pts[1, 0]), int(rotated_pts[1, 1]), int(rotated_pts[2, 0]), int(rotated_pts[2, 1])),
                      fill=(255, 255, 0), width=box_width)
            draw.line((int(rotated_pts[2, 0]), int(rotated_pts[2, 1]), int(rotated_pts[3, 0]), int(rotated_pts[3, 1])),
                      fill=(255, 255, 0), width=box_width)
            draw.line((int(rotated_pts[3, 0]), int(rotated_pts[3, 1]), int(rotated_pts[0, 0]), int(rotated_pts[0, 1])),
                      fill="red", width=box_width)

        if not cls_prob is None:
            score = cls_prob[idx]
            # if idx == 0:
            draw.text((int(rotated_pts[0, 0]-10), int(rotated_pts[0, 1]-10)), str(score), fill=(255, 255, 255, 128), font=font)
...
def vis_image_with_words(img,
    ...
    for idx in range(len(boxes)):
        ...
        if not cls_prob is None:
            score = cls_prob[idx]
            draw.text((int(rotated_pts[0, 0]), int(rotated_pts[0, 1])), str(score)[:5], (255, 0, 122),
                        font)
...
def write_result_ICDAR_RRPN2polys(im_file, dets,
                                  threshold,
                                  result_dir,
                                  height,
                                  width,
    ...
    for idx in range(len(dets)):
        ...
        if not cls_prob is None:
            ...
            if not ms_scores is None:
                det_str = det_str[:-4] + "," + str(ms_scores[idx]) + "\r\n"
            if not scale is None:
                det_str = det_str[:-4] + "," + str(scale[idx]) + "\r\n"
...
def write_result_ICDAR_RRPN2polys_with_words(im_file, dets, word_labels, voc_list, result_dir, height, width):
    ...
    for idx in range(len(dets)):
        ...
        if not voc_list is None:
            ...
            if res_word.upper() == res_word.lower():
                context_word = res_word
            else:
                for i_voc in range(len(voc_list)):
                    dis = editdistance.eval(res_word.upper(), voc_list[i_voc].replace('\ufeff', '').replace('\n', '').upper())
                    edisarr.append(dis)
                ...
                norm_dis = min_dis / float(len(res_word))
...
def write_result_ICDAR_RRPN2ltrb(im_file, dets, threshold, result_dir, height, width, words=None, scores=None):
    file_spl = im_file.split('/')
    file_name = file_spl[len(file_spl) - 1]
    file_name_arr = file_name.split(".")
    ...
    for idx in range(len(dets)):
        ...
        if words is None and not scores is None:
            det_str = str(int(left)) + "," + str(int(top)) + "," + \
                      str(int(right)) + "," + str(int(bottom)) + "," + str(scores[idx]) + "\r\n"
        elif not words is None and not scores is None:
            det_str = str(int(left)) + "," + str(int(top)) + "," + \
                      str(int(right)) + "," + str(int(bottom)) + "," + str(scores[idx]) + "," + words[idx] + "\r\n"
        else:
            det_str = str(int(left)) + "," + str(int(top)) + "," + \
                      str(int(right)) + "," + str(int(bottom)) + "\r\n"

        result_str = result_str + det_str
        return_bboxes.append([left, top, right, bottom])
...
def merge_closest(rboxes, scores, final_thres=0.8):
    ...
    D_boxes = np.sqrt((X - X_) ** 2 + (Y - Y_) ** 2)

    DX_boxes = np.abs(X - X_)
    DY_boxes = np.abs(Y - Y_)

    D_boxes[D_boxes < 1e-6] = 1e4
    DX_boxes[DX_boxes < 1e-6] = 1e4
    DY_boxes[DY_boxes < 1e-6] = 1e4

    # print("D_boxes:", D_boxes)

    closest_arg = np.argmin(D_boxes, axis=1)

    D_boxes_closest = D_boxes[np.arange(rboxes.shape[0]), closest_arg]
    DX_boxes_closest = DX_boxes[np.arange(rboxes.shape[0]), closest_arg]
    DY_boxes_closest = DY_boxes[np.arange(rboxes.shape[0]), closest_arg]

    DX_major = DX_boxes_closest > DY_boxes_closest
    DY_major = ~DX_major

    w_src, h_src, a_src = rboxes[:, 2], rboxes[:, 3], rboxes[:, 4]
    w_tar, h_tar, a_tar = rboxes[closest_arg, 2], rboxes[closest_arg, 3], rboxes[closest_arg, 4]
    S_src, S_tar = scores, scores[closest_arg]

    dis_upper_bound = 1.2 *((w_src + w_tar) / 2 * DX_major + (h_src + h_tar) / 2 * DY_major)

    # 1. Closest distances should be less than 1.4 times of average height or width
    condi_1 = D_boxes_closest < dis_upper_bound
    # 2. Their orientation difference should be less than 5 degree
    condi_2 = np.abs(a_src - a_tar) < 2.
    # 3. Src scores should be less than tar scores
    condi_3 = S_src < S_tar
    # 4. They should have similar width
    # condi_4 = np.abs(w_src - w_tar) < w_src * 0.5

    condi = np.logical_and(np.logical_and(condi_1, condi_2), condi_3)

    S_merge = (S_tar + S_src) / 2. * condi + S_src * (~condi)

    # return rboxes[S_merge > final_thres], S_merge

    return S_merge > final_thres
