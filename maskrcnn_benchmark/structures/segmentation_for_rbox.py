...
class Polygons(object):
    ...
    def shift(self, shift_x, shift_y, new_size, *args, **kwargs):
        # plus to right, minus to left
        shift_polygons = []
        for poly in self.polygons:
            p = poly.clone()
            p[0::2] += shift_x
            p[1::2] += shift_y
            shift_polygons.append(p)

        return Polygons(shift_polygons, size=new_size, mode=self.mode)
...
class SegmentationMask(object):
    ...
    def shift(self, shift_x, shift_y, new_size, *args, **kwargs):
        ...
        # crop into new sizes
        for polygon in self.polygons:
            shifted.append(polygon.shift(shift_x, shift_y, new_size, *args, **kwargs))
        return SegmentationMask(shifted, size=new_size, mode=self.mode)
