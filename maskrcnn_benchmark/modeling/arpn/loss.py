...
import numpy as np
from .geo_target import make_target_rbox, rbox2poly
import cv2
...
class RPNLossComputation(object):
    ...
    def prepare_targets(self, level_objectness, targets, size_range=None, scale=None, level=-1, boxes_reg=None):
        ...
        for targets_per_image, boxes_reg_per_image in zip(targets, boxes_reg):
            ...
            with torch.no_grad():
                ...
                heatmap, regression_targets_per_image = make_target_rbox(
                    (H, W),
                    scale,
                    box_in_4pts,
                    rboxes_cp,
                ...
                if DEBUG:
                    ...
                    if len(rboxes_cp) > 0:
                        ...
                        boxes_reg_per_image_np = boxes_reg_per_image.data.cpu().numpy()

                        print("boxes_reg_per_image_np:", boxes_reg_per_image_np.shape, level_objectness_np.shape, regression_targets_per_image.shape)
        ...
        return torch.cat(labels, dim=0), torch.cat(regression_targets, dim=0)
    ...
    def __call__(self, anchors, objectness, box_regression, targets):
        ...
        for objectness_per_level, box_regression_per_level in zip(
            ...
            label, regression_target = self.prepare_targets(
            ...
            box_regression_per_level = box_regression_per_level.reshape(N, H * W, 5)
