...
class SENet(nn.Module):
    ...
    def _freeze_backbone(self, freeze_at):
        ...
        for stage_index in range(freeze_at):
            if stage_index == 0:
                m = self.layer0  # stage 0 is the stem
            else:
...
def init_pretrained_weights(model, model_url):
    ...
    pretrain_dict_filtered = {}
    for k, v in pretrain_dict.items():

        if k in pretrain_dict:
            if k in model_dict:
                if model_dict[k].size() == v.size():
                    print("loading", k, model_dict[k].size())
                    pretrain_dict_filtered[k] = v
    '''
    pretrain_dict = {
        k: v
        for k, v in pretrain_dict.items()
...
def se_resnext50_32x4d(cfg, loss='softmax', pretrained=True, **kwargs):
    model = SENet(
        cfg=cfg,
        loss=loss,
