...
from maskrcnn_benchmark.modeling.poolers import PyramidRROIAlign, Pooler, PyramidKeepRROIAlign
...
_ROIPOOLER = {
    "RoIRotated": PyramidRROIAlign,
    "RoIRotatedKeep": PyramidKeepRROIAlign
}
...
class ResNet50Conv5RecFeatureExtractor(nn.Module):
    def __init__(self, config):
        ...
        pooler = _ROIPOOLER[config.MODEL.ROI_REC_HEAD.POOLER_TYPE](
             output_size=resolution,
             scales=scales,
             sampling_ratio=config.MODEL.ROI_REC_HEAD.SAMPLING_RATIO
        )
    ...
    def forward(self, x, proposals):
        ...
        if not self.training:
            # The boxes have been rescaled in rbox_head, the rescale factor needs to be updated
            rescale = self.rescale * self.det_margin

        resize_proposals = [proposal.rescale(rescale) for proposal in proposals]
