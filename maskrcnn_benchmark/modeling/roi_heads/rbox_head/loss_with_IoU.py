...
from maskrcnn_benchmark.modeling.matcher import IoUMatcher as Matcher
...
class FastRCNNLossComputation(object):
    ...
    def match_targets_to_proposals(self, proposal, target):
        ...
        matched_targets.add_field("matched_IoU", matched_vals)
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            matched_vals = matched_targets.get_field("matched_IoU")
            ...
            matched_vals_list.append(matched_vals)

        return labels, regression_targets, matched_vals_list
    ...
    def subsample(self, proposals, targets):
        ...
        labels, regression_targets, matched_IoU_list = self.prepare_targets(proposals, targets)
        # print('regression_targets:', targets[0].bbox)
        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels)

        proposals = list(proposals)
        # add corresponding label and regression_targets information to the bounding boxes
        for labels_per_image, regression_targets_per_image, proposals_per_image, matched_IoU in zip(
            labels, regression_targets, proposals, matched_IoU_list
        ):
            proposals_per_image.add_field("labels", labels_per_image)
            proposals_per_image.add_field(
                "regression_targets", regression_targets_per_image
            )
            proposals_per_image.add_field("matched_IoU", matched_IoU)
    ...
    def __call__(self, class_logits, box_regression, IoU_logits):
        ...
        IoU_logits = cat(IoU_logits, dim=0)
        ...
        matched_IoU = cat(
            [proposal.get_field("matched_IoU") for proposal in proposals], dim=0
        )
        if _DEBUG:
            #print('labels:', labels)
        ...
        IoU_logits_pos = IoU_logits[sampled_pos_inds_subset]
        matched_IoU_pos = matched_IoU[sampled_pos_inds_subset]
        ...
        IoU_loss = torch.abs(matched_IoU_pos - IoU_logits_pos).mean()
